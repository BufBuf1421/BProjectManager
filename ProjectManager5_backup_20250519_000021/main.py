    ...
    def select_model(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getOpenFileName(
                ...
                "Выберите модель",
                "",
                "3D Models (*.fbx *.obj);;All Files (*)"
            )
            
            if file_path:
                # Получаем путь к батнику относительно текущей директории
                batch_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "launch_sp.bat")
                
                if not os.path.exists(batch_path):
                    QMessageBox.critical(self, "Ошибка", "Не найден файл launch_sp.bat")
                    return
                    
                try:
                    # Запускаем батник с путем к модели
                    subprocess.Popen([batch_path, file_path])
                    
                    # Закрываем основное приложение
                    self.close()
                    
                except Exception as e:
                    QMessageBox.critical(self, "Ошибка", f"Ошибка при запуске Substance Painter: {str(e)}")
                
        except Exception as e:
            QMessageBox.critical(self, "Ошибка", f"Ошибка при выборе модели: {str(e)}")
